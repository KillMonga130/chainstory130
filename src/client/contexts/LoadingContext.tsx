...
export const LoadingProvider: React.FC<LoadingProviderProps> = ({
  ...
  const updateLoading = useCallback(
    (id: string, updates: Partial<Pick<LoadingState, 'message' | 'progress' | 'type'>>) => {
      setLoadingStates((prev) =>
  ...
  const isLoading = loadingStates.some(
    (state) =>
      state.type === 'loading' ||
...
export const useLoadingOperation = (id: string) => {
  ...
  const start = useCallback(
    (message: string, options?: Parameters<typeof startLoading>[2]) => {
  ...
  const update = useCallback(
    (updates: Parameters<typeof updateLoading>[1]) => {
