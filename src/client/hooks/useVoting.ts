...
import { ApiClient, ClientPerformanceMonitor, ErrorReporting, debounce } from '../utils/error-handler';
...
export const useVoting = (): UseVotingReturn => {
  ...
  const castVote = useCallback(async (chapterId: string, choiceId: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Refresh vote counts (debounced to prevent spam)
        debouncedRefreshVoteCounts(chapterId);
  ...
  const debouncedRefreshVoteCounts = useCallback(
    debounce(refreshVoteCounts, 1000), // 1 second debounce
    [refreshVoteCounts]
  );
