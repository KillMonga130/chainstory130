...
export class StoryHistoryTracker {
  ...
  static async getProgressIndicator(postId: string): Promise<ProgressIndicator | null> {
    try {
      ...
      const remainingChapters = Math.max(0, progression.totalChapters - progression.currentPosition);
  ...
  static async getStoryStatistics(postId: string): Promise<{
    ...
    try {
      ...
      const choiceTypeCounts = choiceTypes.reduce((acc, type) => {
        acc[type] = (acc[type] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);
      
      const mostPopularChoiceEntry = Object.entries(choiceTypeCounts)
        .sort(([,a], [,b]) => b - a)[0];
      const mostPopularChoice = mostPopularChoiceEntry?.[0] || 'unknown';

      // Calculate branch diversity (unique choice patterns)
      const uniquePatterns = new Set(context.pathTaken.map(choice => this.categorizeChoice(choice)));
      ...
      const averageUniqueVoters = history.reduce((sum, entry) => 
        sum + entry.voteStats.uniqueVoters, 0) / history.length;
   ...
   */
  private static analyzeBranchPaths(context: StoryContext, history: StoryHistoryEntry[]): BranchPath[] {
  ...
  private static identifyAlternativeBranches(history: StoryHistoryEntry[]): BranchPath[] {
    ...
    for (const entry of history) {
      if (entry.voteStats.winningPercentage < 60) { // Close decision
  ...
  private static async updateProgressionTracking(
    ...
    try {
      ...
      const updatedProgression = {
        ...
        progressPercentage: Math.round(((progression.currentPosition + 1) / progression.totalChapters) * 100)
      };
   ...
   */
  private static determinePathType(choiceTypes: string[]): 'cautious' | 'bold' | 'investigative' | 'mixed' {
    if (choiceTypes.length === 0) return 'mixed';
    
    const counts = choiceTypes.reduce((acc, type) => {
      acc[type] = (acc[type] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
   ...
   */
  private static determineCompletionStatus(chapterCount: number): 'beginning' | 'middle' | 'advanced' | 'near_end' {
  ...
  private static calculatePathDifficulty(pathTaken: string[]): 'easy' | 'medium' | 'hard' {
    const riskChoices = pathTaken.filter(choice => 
  ...
  private static generatePathDescription(
    ...
    const baseDescription = typeDescriptions[pathType as keyof typeof typeDescriptions] || typeDescriptions.mixed;
